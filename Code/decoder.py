...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, tgt_mask)) # self-attention in decoder side
        # In cross-attention: query comes from decoder's previous layer and Key, value comes from encoder's output
        # Cross-attention uses the mask of the encoder itself not the decoder's
